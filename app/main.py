...
async def process_cancel_command(message: Message):
    await message.answer(
        text='Вы в режиме ожидания. Выберите команду из перечня команд (для вывода команд нажмите /help)'
        )
...
async def process_list_all_command(message: Message):
    ...
    with DbConnect() as db:
        db.cur.execute('SELECT object, meaning FROM bank WHERE user_id = %s ORDER BY object', (message.from_user.id,))
...
LIMIT %s', (message.from_user.id, Limit)
        ...
        await state.update_data(objects = db.cur.fetchall())
...
async def process_training_command(message: Message):
    with DbConnect() as db:
        db.cur.execute('SELECT object, meaning \
FROM bank WHERE user_id = %s and n = 1 \
LIMIT %s', (message.chat.id, Limit)
        )
        learning_pool = db.cur.fetchall()
        if learning_pool:
